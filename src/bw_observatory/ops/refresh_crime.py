...
Invoked as `python -m bw_observatory.ops.refresh_crime` by the scheduler and by
`scripts/refresh_crime.py` for a person at a terminal. Exit codes: 0 done, 1 blocked by the
source or the data, 2 usage, 3 another run holds the lock.
...
def main() -> int:
    ...
    try:
        ...
        print(f"SKIPPED: refresh already running — {exc}")
        return 3
